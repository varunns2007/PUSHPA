...
export const SplitComparisonMap: React.FC<SplitComparisonMapProps> = ({ forestName = 'Nilgiri Biosphere' }) => {
  ...
  return (
    <div className="rounded-xl flex flex-col h-full animate-fade-slide-up" style={{
      ...
      <div
        className="relative mx-4 rounded-xl overflow-hidden flex-1 min-h-[220px]"
        style={{ border: '1px solid rgba(185,28,28,0.25)', background: 'rgba(10,3,0,0.9)' }}
      >
        {/* Dynamic Canvas Background for Before Image */}
        <div
          className="absolute inset-0 overflow-hidden"
          style={{ width: `${sliderPos}%`, borderRight: '2px solid rgba(254,202,202,0.8)' }}
        >
          <div
            ...
            style={{
              width: '100%',
              minWidth: '400px',
              background: 'radial-gradient(ellipse at 30% 40%, rgba(20,83,45,0.85) 0%, rgba(5,46,22,0.95) 70%, rgba(2,20,10,1) 100%)',
            }}
          >
            <div className="flex items-center justify-between z-10">
              <span className="rounded px-2.5 py-1 text-[9px] font-black uppercase tracking-wider" style={{ background: 'rgba(10,3,0,0.85)', color: '#86efac', border: '1px solid rgba(34,197,94,0.5)' }}>
                T1 BEFORE: {dateBefore}
              </span>
              {r && <span className="text-[10px] font-mono-hud font-bold px-2 py-0.5 rounded bg-black/60" style={{ color: '#86efac', border: '1px solid rgba(34,197,94,0.3)' }}>NDVI: {r.ndviBefore}</span>}
            </div>

            <div className="text-left my-auto pl-2 z-10">
              <div className="inline-block px-3 py-1.5 rounded-lg backdrop-blur-md" style={{ background: 'rgba(5,46,22,0.8)', border: '1px solid rgba(34,197,94,0.4)' }}>
                <p className="text-[11px] font-black tracking-wider" style={{ color: '#86efac' }}>🌲 DENSE CANOPY BASELINE</p>
                <p className="text-[9px] font-mono-hud text-green-300/80">Continuous Evergreen Cover (94.6%)</p>
              </div>
            </div>

            <div className="text-[8px] font-mono-hud z-10" style={{ color: 'rgba(134,239,172,0.7)' }}>
              {zoneInfo.lat.toFixed(4)}°N {zoneInfo.lng.toFixed(4)}°E · Cloud: {r?.cloudBefore ?? '4.2'}% · S2B-L2A
            </div>
          </div>
        </div>

        {/* After Side (Revealed when dragging slider) */}
        <div
          className="absolute inset-0 flex flex-col justify-between p-3"
          style={{
            clipPath: `inset(0 0 0 ${sliderPos}%)`,
            background: 'radial-gradient(ellipse at 65% 55%, rgba(127,29,29,0.85) 0%, rgba(69,10,10,0.92) 50%, rgba(20,5,0,0.98) 100%)',
          }}
        >
          <div className="flex items-center justify-between pointer-events-none z-10">
            <span className="rounded px-2.5 py-1 text-[9px] font-black uppercase tracking-wider" style={{ background: 'rgba(10,3,0,0.85)', color: '#fca5a5', border: '1px solid rgba(185,28,28,0.6)' }}>
              T2 AFTER: {dateAfter}
            </span>
            {r && <span className="text-[10px] font-mono-hud font-bold px-2 py-0.5 rounded bg-black/60" style={{ color: '#fca5a5', border: '1px solid rgba(185,28,28,0.4)' }}>NDVI: {r.ndviAfter}</span>}
          </div>

          <div className="text-right my-auto pr-2 z-10">
            <div className="inline-block px-3 py-1.5 rounded-lg backdrop-blur-md text-left" style={{ background: 'rgba(69,10,10,0.85)', border: '1px solid rgba(239,68,68,0.5)' }}>
              <p className="text-[11px] font-black tracking-wider" style={{ color: '#FCA5A5' }}>
                🚨 {r ? `${r.areaHa} HA CLEARING DETECTED` : 'LOGGING CLEARING DETECTED'}
              </p>
              <p className="text-[9px] font-mono-hud text-red-300/80">
                {r ? `Vegetation Loss: -${r.pctLoss}% · Road Access Detected` : 'Exposed Soil & Canopy Drop'}
              </p>
            </div>
          </div>

          <div className="text-[8px] font-mono-hud text-right z-10" style={{ color: 'rgba(252,165,165,0.7)' }}>
            {zoneInfo.lat.toFixed(4)}°N {zoneInfo.lng.toFixed(4)}°E · ΔNDVI: {r ? r.change : '-0.58'} · Sentinel-2
          </div>
        </div>
